...
Deno.serve(async () => {
  ...
  for (const region of regions) {
    try {
      ...
      const to = new Date();
      const demands = await smartgrid.demand({ region, type: 'forecast', from, to });
      if (!demands) throw new Error('Failed to fetch forecast demand');
        ...
        switch (comparator) {
          case 'eq':
            if (demand.value === threshold)
              title = `Forecast System Demand in ${region} is equal to ${threshold}`;
            break;
          case 'neq':
            if (demand.value !== threshold)
              title = `Forecast System Demand in ${region} is not equal to ${threshold}`;
            break;
          case 'lt':
            if (demand.value < threshold)
              title = `Forecast System Demand in ${region} is lesser than ${threshold}`;
            break;
          case 'lte':
            if (demand.value <= threshold)
              title = `Forecast System Demand in ${region} is lesser than or equal to ${threshold}`;
            break;
          case 'gt':
            if (demand.value > threshold)
              title = `Forecast System Demand in ${region} is greater than ${threshold}`;
            break;
          case 'gte':
            if (demand.value >= threshold)
              title = `Forecast System Demand in ${region} is greater than or equal to ${threshold}`;
            break;
        }
      ...
      console.error(err);
      console.error('Exiting due to failed to fetch forecast demand');
