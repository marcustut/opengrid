...
Deno.serve(async () => {
  console.log('Running forecast demand alert');
  ...
  for (const region of regions) {
    try {
      ...
      const to = new Date();
      console.log('Fetching forecast demand for ', region);
      const demands = (await smartgrid.demand({ region, type: 'forecast', from, to }))?.filter(
        (demand) => !!demand.Value,
      );
      if (!demands) throw new Error('Failed to fetch forecast demand');
      const demand = demands[demands.length - 1];
      console.log('Current forecast demand: ', demand.Value);
